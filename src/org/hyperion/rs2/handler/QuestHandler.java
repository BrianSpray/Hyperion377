...
public abstract class QuestHandler {
	...
	public static void sendQuestInterface(final Player player, final int QUEST_ID, final String[][] QUEST_LINES) {
		/*Action action = new Action("QUEST_LOGS") {

			@Override
			public CancelPolicy[] getCancelPolicy() {
				return new CancelPolicy[] {CancelPolicy.WALKING, CancelPolicy.CLOSE_INTERFACE, CancelPolicy.ATTACK, CancelPolicy.DEATH};
			}
			...
			@Override
			public void execute() {
				player.addTemporary("BUSY", true);
				int stage = player.getQuests().getQuest(QUEST_ID).getStage();
				String name = player.getQuests().getQuestInfo(QUEST_ID).getName();
				int count = 4;
				String[] lines = QUEST_LINES[stage];
				for (int i = 0; i < lines.length; i++) {
					if (!lines[i].contains("::")) {
						ActionSender.sendString(player, lines[i], 275, count++);
						continue;
					}
					String line = "";
					String[] commands = lines[i].split("::");
					for (int l = 0; l < commands.length; l++) {
						commands[l] = commands[l].replace("::", "").replace("COMMAND", "");
						if (commands[l].contains("::")) {
							line += commands[l].replace("::", "");
						} else {
							line += handleCommand(player, commands[l].split(":"));
						}
					}
					ActionSender.sendString(player, line, 275, count++);
				}
				for (int i = count; i < 137; i++) {
					ActionSender.sendString(player, "", 275, i);
				}
				ActionSender.sendString(player, "<col=800000>"+name+"</col>", 275, 2);
				ActionSender.sendInterface(player, 275);
			}

			@Override
			public void cancel() {
				player.removeTemporary("BUSY");
				ActionSender.sendUpdateItems(player, 12, 89, -1, player.getBank().getItems());
				ActionSender.sendUpdateItems(player, 15, 0, 93, player.getInventory().getItems());
				ActionSender.sendInterface(player, 12);
				ActionSender.sendInventoryInterface(player, 15);
			}
		};
		ActionManager.appendAction(player, action);*/
	}

	private static String handleCommand(Player player, String[] command) {
		/*if (command[0].equals("CONTAINS")) {
			if (player.getInventory().contains(new Item(Integer.parseInt(command[1]), Integer.parseInt(command[2])))) {
				return command[3];
			} else {
				return command[4];
			}
